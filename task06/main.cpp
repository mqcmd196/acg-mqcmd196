...
void search_collision_in_bvh(
  ...
  if (!bvhnodes[i_bvhnode].intersect_bv(ray_org, ray_dir)) { // no intersection
      return;
  }
...
//             sum += 1.f; // Problem 3: This is a bug. write some correct code (hint: use `dir.dot(nrm)`, `pdf`, `M_PI`).
             const float cos_theta = dir.dot(nrm);
             sum += cos_theta / pdf / float(M_PI);
          }
        }
